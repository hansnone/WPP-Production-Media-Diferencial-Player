...
impl Default for TpState {
    fn default() -> Self {
        Self { peak: -60.0, age: 0.0, clip: false, ppm_level: 0.0 }
    }
}
...
impl TpState {
    fn update(&mut self, tp: f32, dt: f32) {
        ...
        let tp_db = if tp <= 0.00001 { -60.0 } else { 20.0 * tp.log10() };
        ...
        let tau = if tp > self.ppm_level { TAU_ATTACK } else { TAU_RELEASE };
...
static TP_STATE: Mutex<[[TpState; 2]; 2]> = Mutex::new([[TpState { peak: -60.0, age: 0.0, clip: false, ppm_level: 0.0 }; 2]; 2]);
...
fn lufs_color(lufs: f32, lit: bool) -> Color32 {
    ...
    if lit {
        ...
        Color32::from_rgb((r as f32 * 0.08) as u8, (g as f32 * 0.08) as u8, (b as f32 * 0.08) as u8)
    }
}
...
fn draw_ebu_panel(
    ...
    painter.rect_stroke(bg, Rounding::same(4.0), Stroke::new(1.0, Color32::from_rgb(35, 35, 50)));

    // Left Column: PPM (True Peak Fast)
    let ppm_val_l_db = if tp_l.ppm_level <= 0.00001 { -60.0 } else { 20.0 * tp_l.ppm_level.log10() };
    let ppm_val_r_db = if tp_r.ppm_level <= 0.00001 { -60.0 } else { 20.0 * tp_r.ppm_level.log10() };
    ...
    for i in 0..NUM_LEDS {
        ...
        let rect_l_l = Rect::from_min_size(Pos2::new(col_l_x, y), Vec2::new(LED_W / 2.0 - 1.0, LED_H));
        let rect_l_r = Rect::from_min_size(Pos2::new(col_l_x + LED_W / 2.0 + 1.0, y), Vec2::new(LED_W / 2.0 - 1.0, LED_H));
        let rect_r = Rect::from_min_size(Pos2::new(col_r_x, y), Vec2::new(LED_W, LED_H));

        // Draw PPM L/R on left, LUFS on right
        painter.rect_filled(rect_l_l, Rounding::same(1.0), ppm_color(led_val, ppm_val_l_db >= led_val));
        painter.rect_filled(rect_l_r, Rounding::same(1.0), ppm_color(led_val, ppm_val_r_db >= led_val));
        painter.rect_filled(rect_r, Rounding::same(1.5), lufs_color(led_val, m_lufs >= led_val));
    }

    // Target reference line at -23 (Target LUFS)
    let ref_y = db_to_y(-23.0) + LED_H / 2.0;
    painter.line_segment(
        [Pos2::new(col_r_x - 4.0, ref_y), Pos2::new(col_r_x + LED_W + 4.0, ref_y)],
    ...
    painter.line_segment(
        [Pos2::new(col_l_x - 4.0, tp_ref_y), Pos2::new(col_l_x + LED_W + 4.0, tp_ref_y)],
        Stroke::new(2.0, Color32::from_rgb(255, 50, 50)),
    );

    // ── True Peak markers ────────────────
    let draw_tp_marker = |x: f32, w: f32, peak_db: f32| {
        if peak_db <= -53.5 { return; }
        ...
        if peak_db >= -1.0 { color = Color32::from_rgb(255, 0, 0); }
    ...
    for &db in MARKS {
        ...
        painter.line_segment(
            [Pos2::new(scale_x + SCALE_W - 5.0, y), Pos2::new(scale_x + SCALE_W, y)],
            Stroke::new(1.0, Color32::from_rgb(70, 70, 90)),
        );
        ...
        painter.text(
            ...
            if db == -23.0 { Color32::from_rgb(0, 255, 255) } else { Color32::from_rgb(150, 150, 170) },
        );
    }
    ...
    let clip_resp = ui.interact(cr_l, ui.id().with(format!("clip_{}", ch_idx)), egui::Sense::click());
    ...
    painter.rect_stroke(cr_l, Rounding::same(2.0), Stroke::new(0.5, Color32::from_rgb(80, 30, 30)));
    painter.text(cr_l.center(), egui::Align2::CENTER_CENTER, "OVER", FontId::monospace(7.5), text_col);

    // ── Headers ───────────────────────────────────────────────────────────────
    let header_label_y = origin.y + 18.0;
    painter.text(Pos2::new(col_l_x + LED_W / 2.0, header_label_y), egui::Align2::CENTER_CENTER, "PPM", FontId::monospace(12.0), Color32::from_rgb(255, 120, 120));
    painter.text(Pos2::new(col_r_x + LED_W / 2.0, header_label_y), egui::Align2::CENTER_CENTER, "LUFS", FontId::monospace(12.0), Color32::from_rgb(120, 190, 255));

    // ── Footer: numeric readouts ──────────────────────────────────────────────
    let mut footer_y = leds_top + col_h + 8.0;

    let fmt_val = |v: f32| -> String {
        if !v.is_finite() || v <= -119.5 { " -∞  ".into() } else { format!("{:5.1}", v) }
    };

    let max_ppm = ppm_val_l_db.max(ppm_val_r_db);
    painter.text(Pos2::new(col_l_x + LED_W / 2.0, footer_y), egui::Align2::CENTER_TOP, fmt_val(max_ppm), FontId::monospace(12.0), ppm_color(max_ppm, true));
    painter.text(Pos2::new(scale_x + SCALE_W / 2.0, footer_y), egui::Align2::CENTER_TOP, "dB/LU", FontId::monospace(9.0), Color32::from_rgb(70, 70, 90));
    painter.text(Pos2::new(col_r_x + LED_W / 2.0, footer_y), egui::Align2::CENTER_TOP, fmt_val(m_lufs), FontId::monospace(12.0), lufs_color(m_lufs, true));

    footer_y += 20.0;
    // Short-term & Integrated
    let s_lufs = loudness.short_term as f32;
    let i_lufs = loudness.integrated as f32;
    painter.text(Pos2::new(col_l_x, footer_y), egui::Align2::LEFT_TOP, "Short-term", FontId::monospace(11.0), Color32::from_rgb(150, 150, 170));
    painter.text(Pos2::new(col_r_x + LED_W, footer_y), egui::Align2::RIGHT_TOP, format!("{} LUFS", fmt_val(s_lufs)), FontId::monospace(12.0),
        if (s_lufs + 23.0).abs() <= 2.0 { Color32::from_rgb(50, 255, 100) } else { Color32::from_rgb(255, 200, 50) });

    footer_y += 16.0;
    painter.text(Pos2::new(col_l_x, footer_y), egui::Align2::LEFT_TOP, "Integrated", FontId::monospace(11.0), Color32::from_rgb(150, 150, 170));
    painter.text(Pos2::new(col_r_x + LED_W, footer_y), egui::Align2::RIGHT_TOP, format!("{} LUFS", fmt_val(i_lufs)), FontId::monospace(12.0),
        if (i_lufs + 23.0).abs() <= 1.0 { Color32::from_rgb(50, 255, 100) } else { Color32::from_rgb(255, 200, 50) });

    footer_y += 16.0;
    // True Peak Max
    let max_tp = tp_l.peak.max(tp_r.peak);
    painter.text(Pos2::new(col_l_x, footer_y), egui::Align2::LEFT_TOP, "True Peak", FontId::monospace(11.0), Color32::from_rgb(150, 150, 170));
    painter.text(Pos2::new(col_r_x + LED_W, footer_y), egui::Align2::RIGHT_TOP, format!("{} dBTP", fmt_val(max_tp)), FontId::monospace(12.0),
        if max_tp >= -1.0 { Color32::from_rgb(255, 50, 50) } else { Color32::from_rgb(50, 200, 255) });
}
