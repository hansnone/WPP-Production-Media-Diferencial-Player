...
pub fn show(ui: &mut Ui, app: &mut DiffPlayerApp) {
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        ui.label(RichText::new(if is_es { "Control de Calidad Frame a Frame" } else { "Frame-Accurate Video QC" }).size(11.0).weak());
        ui.separator();
        ...
        egui::Grid::new("playback_grid")
            ...
            .show(ui, |ui| {
                ...
                kv(ui, if is_es { "Cuadro" } else { "Frame" }, &frame_n.to_string());
                kv(ui, if is_es { "Zoom" } else { "Zoom" },  &format!("{zoom:.2}×"));
                kv(ui, if is_es { "Paneo" } else { "Pan UV" },&format!("({pan_u:.3}, {pan_v:.3})"));
            });
        ...
        let color_a = if dark_mode { Color32::from_rgb(100, 200, 120) } else { Color32::from_rgb(20, 110, 50) };
        let color_b = if dark_mode { Color32::from_rgb(100, 160, 240) } else { Color32::from_rgb(30, 70, 180) };
        ...
        if let (Some(ma), Some(mb)) = (meta_a.as_ref(), meta_b.as_ref()) {
                ...
                || ma.color_primaries != mb.color_primaries
            {
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ui.label(
                            RichText::new(if is_es { "⚠ ¡Discrepancia de metadatos de color!" } else { "⚠ Color metadata mismatch detected!" })
                        ...
                        if ma.colorspace != mb.colorspace {
                            ui.label(
                                RichText::new(if is_es { format!("  Espacio: {} ≠ {}", ma.colorspace, mb.colorspace) } else { format!("  Colorspace: {} ≠ {}", ma.colorspace, mb.colorspace) })
                                .size(10.5)
                        ...
                        if ma.color_transfer != mb.color_transfer {
                            ui.label(
                                RichText::new(if is_es { format!("  Transferencia: {} ≠ {}", ma.color_transfer, mb.color_transfer) } else { format!("  Transfer: {} ≠ {}", ma.color_transfer, mb.color_transfer) })
                                .size(10.5)
                                .color(Color32::LIGHT_GRAY),
                            );
                        }
                        if ma.color_primaries != mb.color_primaries {
                            ui.label(
                                RichText::new(if is_es { format!("  Primarios: {} ≠ {}", ma.color_primaries, mb.color_primaries) } else { format!("  Primaries: {} ≠ {}", ma.color_primaries, mb.color_primaries) })
                                .size(10.5)
                                .color(Color32::LIGHT_GRAY),
                            );
                        }
        ...
        // ── Usage hints ───────────────────────────────────────────────────
        ui.separator();
        ui.label(RichText::new(if is_es { "Atajos" } else { "Shortcuts" }).size(11.0).strong().weak());
        ...
        egui::Grid::new("hints_grid")
            ...
            .show(ui, |ui| {
                for (key, desc) in hints {
                    ui.label(RichText::new(key).monospace().size(10.5).color(Color32::from_rgb(150, 200, 255)));
...
fn channel_section(
    ...
    if let Some(path) = path {
        ...
        ui.label(RichText::new(if is_es { "Ningún archivo cargado" } else { "No file loaded" }).weak().italics().size(11.0));
        return;
    }
    ...
    if let Some(m) = meta {
        ...
        egui::Grid::new(format!("meta_{label}"))
            ...
            .spacing([8.0, 2.0])
            .show(ui, |ui| {
                kv(ui, if is_es { "Resolución" } else { "Resolution" }, &format!("{}×{}", m.width, m.height));
                kv(ui, "FPS",        &format!("{:.4}", m.fps));
                kv(ui, if is_es { "Duración" } else { "Duration" },   &format_dur(m.duration_secs));
                kv(ui, if is_es { "Tasa bits" } else { "Bitrate" },    &format!("{} kbps", m.bitrate_kbps));
                kv(ui, if is_es { "Fmt Píxel" } else { "Pixel Fmt" },  &m.pixel_format);
                kv(ui, if is_es { "Espacio" } else { "Colorspace" }, &m.colorspace);
                kv(ui, if is_es { "Transfer" } else { "Transfer" },   &m.color_transfer);
                kv(ui, if is_es { "Primarios" } else { "Primaries" },  &m.color_primaries);
            });
    }
}
